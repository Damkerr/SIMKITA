...
const DOKUMEN_SAKIP = [
  ...
  {id:3,nama:"Pakta Integritas OPD",komponen:"Perencanaan Kinerja",bobot:4,bulan:0,tw:"TW 1"},
  ...
  {id:8,nama:"Perjanjian Kinerja OPD",komponen:"Perencanaan Kinerja",bobot:3,bulan:1,tw:"TW 1"},
...
export default function App() {
  ...
  // Load data awal dari Firestore + subscribe realtime
  useEffect(()=>{
    (async()=>{
      try{
        const d=await loadData();
        setAppData(d||initData());
      }catch(e){
        console.error("Gagal load Firebase:",e);
        setAppData(initData());
      }
      setLoading(false);
      // Subscribe perubahan realtime dari pengguna lain
      unsubRef.current=subscribeData((d)=>{
        setAppData(d);
      });
    })();
  ...
  const save=useCallback(async(nd)=>{
    ...
    try{
      ...
      console.error("Gagal simpan Firebase:",e);
      setSyncStatus("error");
      ...
      <main style={{marginLeft:ml,minHeight:"100vh",background:"#f0f4f8",transition:"margin-left .25s"}}>
        <div style={{background:"#fff",borderBottom:"1px solid #e8edf8",padding:"11px 26px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:50,boxShadow:"0 1px 4px rgba(0,0,0,.05)"}}>
          ...
          <div style={{display:"flex",gap:14,alignItems:"center",fontSize:12,color:"#64748b"}}>
            {syncStatus==="saving"&&<span style={{color:"#f39c12",fontWeight:600}}>⏳ Menyimpan...</span>}
            {syncStatus==="saved"&&<span style={{color:"#1e8449",fontWeight:600}}>✅ Tersimpan</span>}
            {syncStatus==="error"&&<span style={{color:"#c0392b",fontWeight:600}}>❌ Gagal simpan</span>}
            {syncStatus==="idle"&&appData?.lastUpdate&&<span>Update: {new Date(appData.lastUpdate).toLocaleString("id-ID")}</span>}
